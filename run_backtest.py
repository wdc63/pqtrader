...
if __name__ == '__main__':
    ...
    pause_simulation_pkl_path = os.path.join(current_dir, 'strategies', 'simple_ma', 'simulation', '20251019_150607', 'MyStrategy_pause.pkl')
    ...
    print("\n--- 模式2: 恢复回测 (直接运行) ---")
    BacktestRunner.run_resume(
        state_file=pause_simulation_pkl_path,
        config_path=config_simulation_path,
        # config_path=config_path,   # 可以换成一个新的配置文件，可选
        # data_provider_path=data_provider_path,  # 可以换成一个新的数据源，可选
        start_paused=True     # 恢复后是否立即暂停，可选，默认False
    )
