...
class Position:
    ...
    def __init__(
        ...
        Args:
            ...
            current_dt: 建仓时间
        """
        self.symbol = symbol  # 标的标识符
        self.symbol_name = symbol_name # 股票名称
...
class AbstractDataProvider(ABC):
    ...
    def get_current_snapshot(
        ...
        Args:
            symbol: 标的标识符，例如 '000001' 或 '600519.SH'`
            dt: 查询时间（精确到秒）
...
class MatchingEngine:
    ...
    def _is_t_plus_1(self, symbol: str) -> bool:
        ...
        """
        return self.context.config.get('trading_rule') == 'T+1'
```
...
class Scheduler:
    ...
    def _build_tick_schedule(self) -> list:
        ...
        for session_start_str, session_end_str in self.trading_sessions:
            ...
            # 创建一个日期用于时间运算
            base_date = datetime(2023, 1, 1)  # 任意日期，仅用于时间计算
            ...
            while current_dt <= end_dt:
                ...
                current_dt += timedelta(seconds=3)
...
*   **职责**:
    *   在策略初始化时，通过 `data_provider.get_history_bars` **一次性获取**基准标的在回测期内的**全部日线收盘价**并缓存于内存。
...
# ========== 基准配置 ==========
benchmark:
  symbol: "000300.XSHG"  # 基准标的代码（'000300'=沪深300）
...
- [x] 数据接口合约定义（AbstractDataProvider，移除get_history_bars）
- [x] 订单、持仓、账户模型（包含即时/历史订单区分）
