...
class Engine:
        ...
        if config_path is None:
            ...
            if config_snapshot.exists():
                ...
                raise FileNotFoundError(
                    f"无法从状态文件 '{state_file}' 加载：缺少对应的配置文件快照 'snapshot_config.yaml'。"
                    "请确保状态文件和配置文件快照在同一个目录下。"
                )
    ...
    def _run_simulation_unified(self, strategy_path: Optional[str], data_provider_path: Optional[str], is_resume: bool, start_paused: bool = False):
        ...
        if is_resume:
            ...
            self.workspace_manager = WorkspaceManager(strategy_path, data_provider_path, self.config, self.temp_logger, mode=self.context.mode, config_path=self.config_path)
